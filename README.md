...
- **核心工作流**: Sweep -> Evaluate -> Ablation -> Sensitivity
...
- **不可变沙盒隔离**: sweep 结束后发布 W&B source artifact，evaluate/ablation/sensitivity 在 `/tmp/lightning-runs/...` 沙盒中执行 payload；同 sweep id 默认拒绝覆盖 source artifact
...
`evaluate` / `ablation` / `sensitivity` 会发送邮件；`sweep` 只创建 sweep、等待 agent 并发布 source artifact。邮件均包含完整 Sweep ID、Run URL、最优配置 (`rank N best`)、测试指标、复现脚本 (`seed=[42,43,44]` 格式)、Sweep URL。
...
2026-05-22 真实调试覆盖了 `sweep / evaluate / ablation / sensitivity`。`evaluate`、`ablation`、`sensitivity` 的邮件均确认能发送并落盘到 `logs/mail/{mode}/`；邮件标题/文件名使用完整 sweep id，ablation/sensitivity 的 rank 报告与邮件路径带 `_rankN`。
...
## Source Version Safety

`scripts/rsync.sh` 同步的是当前工作区源码；W&B `code_sweep_<sweep_id>` artifact 才是某个 sweep 的训练源码真相。新 sweep 会绑定新 artifact。旧 sweep 的 evaluate/ablation/sensitivity 会继续拉取旧 artifact，所以远端同步新代码后不会污染旧 sweep 复现。默认 `workflow.snapshot.allow_source_overwrite=false`，同 sweep id 再发布 source artifact 会失败。
